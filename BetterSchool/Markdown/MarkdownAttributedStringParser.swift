...
public struct MarkdownAttirbutedStringParser: MarkupVisitor {
    ...
    mutating func parserResults(from document: Document) -> [ParserResult]{
        ...
        func appendCurrentAttributedString(){
            if !currentAttrString.string.isEmpty{
                let currentStringToAppend = (try? AttributedString(currentAttrString, including: \.uiKit))
                ?? AttributedString(stringLiteral: currentAttrString.string)
                results.append(.init(attributedString: currentStringToAppend, isCodeBlock: false, codeBlockLanguage: nil))
            }
